...
OUTPUT_JSONL_PATH = 'logs/debug/full_cot_tool.jsonl'
DEBUG_LOOP_TOTAL = 3
...
def rag_constraint_search(query, num_extraction=6):
    ...
    '''
    results = search_constraints("app-CRG", query, num_extraction)
...
def rag_tool_search(query, num_extraction=1):
    ...
    It assumes index is already created and uploaded.
    '''
    results = search_tools("app-CRG", query, num_extraction)
...
def main():
    ...
    prompt_list = [
        # 8 easy ones
        # "How many nodes have type 'virtualmachines' in the graph? Return only the number.",
        # "How many links exist in the graph? Return only the number.",
        # "Show the first ten 'virtualmachines' type nodes ordered by name in descending order. Return a list with node 'name'.",
        # "Show first five 'virtualmachines' type nodes by their name and their 'osType'. Return a table with header ['VM name', 'OS type'].",
        # "Count 'virtualmachines' type nodes by 'osType'. Return a table with header ['OS type', 'Count'].",
        # "How many 'Networkinterfaces' type nodes have properties 'virtualnetworks' as 'Subnet-1'. Return only the number",
        ...
        #
        # # 8 medium ones
        # "How many nodes are there have links to 'networksecuritygroups' type nodes? Return only the number.",
        # "How many isolated nodes without any links exist in the graph? Return only the number.",
        # "Find all nodes that can be connected to 'virtualnetworks' type nodes with 'addressPrefixes' as '10.0.0.1' and 'port' as 26. Return the node 'name' in a list.",
        # "With all 'virtualnetworks' type nodes that have name 'Subnet-2', list the top five node degrees. Return a list of assorted degree.",
        # "How many unique nodes have edges to 'networksecuritygroups' type nodes with 'port' 23? Return only the number.",
        # "Show me the unique 'addressPrefixes' for 'virtualnetworks' type nodes and the number of nodes per prefix. Sort them based on number of nodes. Return a table without headers.",
        # "List all of 'networksecuritygroups' type nodes with protocol 'TCP'. Return a list with node 'name'.",
        # "Cut the graph into two parts such that the number of 'virtualnetworks' type nodes between the cuts is same. Color two parts with red and blue. Return the networkx graph object.",
        #
        # # 4 hard ones
        # "How many nodes except from type 'networksecuritygroups' have more than ten links in the graph? Return only the number.",
        "Extract all 'virtualmachines' type nodes with 'osType'='linux' and their connected nodes with links. Return the new graph.",
        "Count 'networksecuritygroups' type nodes that are related to inbound traffic ('InBound' is contained as part of its 'name'). Return only the number.",
        "Identify all paths from the 'Networkinterfaces' type nodes to 'virtualnetworks' type node with 'name' as 'Subnet-1', and rank them based on the lowest number of hops. networkx is imported as nx. Return a list.",
        ]
