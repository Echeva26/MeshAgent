...
def rag_vector_search(query, num_extraction=9):
    '''
...
def userQuery(prompt_list):
    ...
    for each_prompt in prompt_list:
        ...
        for i in range(EACH_PROMPT_RUN_TIME):
            ...
            print("Calling model")
            answer = constraint_only_chain.run({'input': requestData['query'],
            ...
            if ground_truth_ret['type'] == 'text':
                ...
                ground_truth_graph = nx.DiGraph(ground_truth_ret['data'])
                ret_graph = nx.DiGraph(ret_graph_copy)
